...
class VaultEvents {
  VaultEvents(this.plugin);

  final PluginHandle plugin;

  Stream<VaultEvent> get all => _all.stream;
  Stream<VaultEvent> get created =>
      _all.stream.where((e) => e.type == VaultEventTypes.create);
  Stream<VaultEvent> get modified =>
      _all.stream.where((e) => e.type == VaultEventTypes.modify);
  Stream<VaultEvent> get deleted =>
      _all.stream.where((e) => e.type == VaultEventTypes.delete);
  Stream<VaultEvent> get renamed =>
      _all.stream.where((e) => e.type == VaultEventTypes.rename);
  ...
  void attach() {
    ...
    void register(
      String event,
      VaultEvent Function(List<Object?> args) mapper,
    ) {
    ...
    register('create', (args) => VaultEvent.create(_asFile(args[0])));
    register('modify', (args) => VaultEvent.modify(_asFile(args[0])));
    register('delete', (args) => VaultEvent.delete(_asAbstractFile(args[0])));
    register(
      'rename',
...
class VaultEvent {
  ...
  factory VaultEvent.create(TFileHandle file) =>
      VaultEvent._(VaultEventTypes.create, file);

  factory VaultEvent.modify(TFileHandle file) =>
      VaultEvent._(VaultEventTypes.modify, file);

  factory VaultEvent.delete(TAbstractFileHandle file) =>
      VaultEvent._(VaultEventTypes.delete, file);
...
TAbstractFileHandle _asAbstractFile(Object? value) =>
    TAbstractFileHandle(value! as JSObject);
